...
class PseudoGraph {
     ...
     */
    void add(int value, boolean afterClose) {
        if (!afterClose) beforeClose = graphList.size();
    ...
    public String toString() {
        ...
        for (int y = 2 * (maxValue / 2); y >= 0; y -= 2) {
            ...
            for (int x = 0; x < graphList.size(); x++) {
                ...
                else if (graphList.get(x) >= y) retValue.append(x <= beforeClose ? "█" : "░");
                else retValue.append("▄");
            }
            retValue.append("\n");
        }
        retValue.append("    ");
